...
| `subsystems/runtime/request-tracing.md` | Request tracing first slice implemented behind `--request-tracing`: the vLLM bridge carries an opt-in trace through `GenerateRequest`, Qwen3 and `openinfer-sim` record scheduler/forward events, and terminal summaries reuse the benchmark-compatible `openinfer_http_trace` JSON log contract. |
